...
export const ProductionPurchasesView:React.FC=()=>{
 ...
 return <div className="flex-1 p-3 sm:p-4 lg:p-6 overflow-y-auto space-y-5 bg-neutral-950 text-white">
   <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-neutral-800">
     <div><h1 className="text-xl font-black flex items-center gap-2"><ShoppingBag size={22} className="text-amber-400"/>Compras & Entradas de Mercadorias</h1><p className="text-xs text-neutral-400 mt-1">Recebimento, nota, lotes, validade, custo, troca de preço, estoque e contas a pagar.</p></div>
     <div className="flex gap-2"><button disabled={busy} onClick={()=>void load()} className="px-3 py-2 rounded-xl border border-neutral-700 text-xs text-neutral-300 flex items-center gap-2"><RefreshCw size={14}/>Atualizar</button><button disabled={busy||products.length===0} onClick={start} className="px-4 py-2 rounded-xl bg-amber-500 text-neutral-950 text-xs font-black flex items-center gap-2"><Plus size={14}/>Nova entrada</button></div>
   </div>
     ...
     <form onSubmit={save} className="p-4 overflow-y-auto space-y-5">
       ...
       <div className="space-y-2">
         ...
         {items.map((it,idx)=>{const p=products.find(x=>x.id===it.productId);return <div key={idx} className="grid lg:grid-cols-[2fr_.7fr_1fr_1fr_1fr_1fr_38px] gap-2 p-3 rounded-xl bg-neutral-950 border border-neutral-800">
           ...
           <input title="Novo preço de venda" placeholder={`Preço atual ${p?.salePrice?.toFixed(2)||'0.00'}`} type="number" min="0" step="0.01" value={it.newSalePrice} onChange={e=>patchItem(idx,{newSalePrice:e.target.value})} className="bg-neutral-900 border border-neutral-700 rounded-xl px-2 py-2 text-xs"/>
