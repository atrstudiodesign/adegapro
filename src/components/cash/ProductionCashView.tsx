...
export const ProductionCashView: React.FC<Props> = ({ currentUser, currentSession, onSessionUpdated }) => {
  ...
  return <div className="flex-1 p-3 sm:p-4 lg:p-6 overflow-y-auto space-y-5 bg-neutral-950">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-neutral-800">
      <div><h1 className="text-xl font-black text-white flex items-center gap-2"><Wallet size={22} className="text-amber-400"/>Caixa de Produção</h1><p className="text-xs text-neutral-400 mt-1">Operações executadas por transações seguras no Supabase.</p></div>
      <button disabled={busy} onClick={()=>void load()} className="px-3 py-2 rounded-xl border border-neutral-700 text-xs text-neutral-300 flex items-center gap-2"><RefreshCw size={14}/>Atualizar</button>
    </div>
    ...
    {!currentSession ? <div className="p-5 rounded-2xl bg-neutral-900 border border-neutral-800"><h2 className="font-bold text-white">Nenhuma sessão aberta para {currentUser.name}</h2><button disabled={busy} onClick={open} className="mt-4 px-5 py-2.5 rounded-xl bg-amber-500 text-neutral-950 font-black text-xs flex items-center gap-2"><Unlock size={15}/>Abrir caixa</button></div> :
    <div className="p-5 rounded-2xl bg-neutral-900 border border-neutral-800 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3"><Metric label="Saldo inicial" value={currentSession.initialBalance}/><Metric label="Vendas dinheiro" value={currentSession.totalCashSales}/><Metric label="PIX" value={currentSession.totalPixSales}/><Metric label="Cartões" value={currentSession.totalCardDebitSales+currentSession.totalCardCreditSales}/></div>
    ...
    <div className="grid lg:grid-cols-2 gap-4">
      ...
      <section className="p-4 rounded-2xl bg-neutral-900 border border-neutral-800"><h2 className="text-sm font-bold text-white mb-3">Últimas sessões</h2><div className="space-y-2">{sessions.slice(0,8).map(s=><div key={s.id} className="flex justify-between gap-3 text-xs border-b border-neutral-800 pb-2"><span>{s.cashRegisterNumber} · {new Date(s.openedAt).toLocaleString('pt-BR')}</span><span className={s.status==='ABERTO'?'text-emerald-400':'text-neutral-500'}>{s.status}</span></div>)}</div></section>
    </div>
  </div>;
};

const Metric=({label,value}:{label:string;value:number})=><div className="p-3 rounded-xl bg-neutral-950 border border-neutral-800"><div className="text-[10px] text-neutral-500 uppercase">{label}</div><div className="font-mono font-bold text-white mt-1">R$ {value.toFixed(2)}</div></div>;