...
export const CommercialAccessGate:React.FC<{onAllowed:()=>void}>=({onAllowed})=>{
 ...
 useEffect(()=>{
   let alive=true;
   productionDb.getEntitlement().then(async data=>{
     if(!alive)return;
     if(data?.active){onAllowed();return;}
     if(data?.mode==='PLATFORM_ADMIN_ACCOUNT'){
       await supabase.auth.signOut();
       window.location.replace('/atr-control');
       return;
     }
     if(data?.mode==='NO_TENANT'){
       await supabase.auth.signOut();
       window.location.replace('/entrar');
       return;
     }
     setState(data);
   }).catch(e=>alive&&setError(e?.message||'Não foi possível validar a licença comercial.'));
   return()=>{alive=false;};
 },[onAllowed]);
